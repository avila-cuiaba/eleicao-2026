// Página parcerias: contratos por liderança/município + filtro por região (planilha municipios).
...
const CAMPOS_PLANILHA = [
  ...
  {
    prop: "parceriaLider",
    chave: "PARCERIA_LIDER",
    aliases: ["parceria-lider", "parceria lider", "apoiador-lider", "apoiador lider"],
  },
  {
    prop: "parceria30",
    chave: "PARCERIA_30",
    aliases: ["parceria-30", "parceria 30", "apoiador-30", "apoiador 30"],
  },
  {
    prop: "parceria45",
    chave: "PARCERIA_45",
    aliases: ["parceria-45", "parceria 45", "apoiador-45", "apoiador 45"],
  },
  {
    prop: "parceriaCustomizado",
    chave: "PARCERIA_CUSTOMIZADO",
    aliases: [
      "parceria-customizado",
      "parceria customizado",
      "parceria-livre",
      "parceria livre",
...
function extrairLinhas(valores) {
  ...
  for (let i = cfgPar.LINHA_INICIO_DADOS - 1; i < valores.length; i++) {
    ...
    const item = {
      ...
      municipio,
      parceriaLider: valorCampo(linha, indices.parceriaLider),
      parceria30: valorCampo(linha, indices.parceria30),
      parceria45: valorCampo(linha, indices.parceria45),
      parceriaCustomizado: valorCampo(linha, indices.parceriaCustomizado),
      regiao: info?.regiao || "",
...
function atualizarKpis(filtradas) {
  ...
  el.kpiLider.textContent = fmt.format(somarCampo(filtradas, "parceriaLider"));
  el.kpi30.textContent = fmt.format(somarCampo(filtradas, "parceria30"));
  el.kpi45.textContent = fmt.format(somarCampo(filtradas, "parceria45"));
  el.kpiCustom.textContent = fmt.format(somarCampo(filtradas, "parceriaCustomizado"));
}
...
function largurasColunasTabela() {
...
function sincronizarLargurasColunas(headTable, bodyTable) {
  const mobile = window.matchMedia("(max-width: 991.98px)").matches;
  const larguras = largurasColunasTabela();
...
function renderizarLinha(r) {
  ...
  return `<tr>
    ...
    <td class="text-end apoiadores-col-lider apoiadores-col-separador apoiadores-celula-num">${exibirCelula(r.parceriaLider)}</td>
    <td class="text-end apoiadores-col-30 apoiadores-celula-num">${exibirCelula(r.parceria30)}</td>
    <td class="text-end apoiadores-col-45 apoiadores-celula-num">${exibirCelula(r.parceria45)}</td>
    <td class="text-end apoiadores-col-custom apoiadores-celula-num">${exibirCelula(r.parceriaCustomizado)}</td>
  </tr>`;
}
